...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // NEW: Clean Daily Scheduling endpoint (separate from other functionality)
  app.get('/api/scheduling/daily/:date', async (req, res) => {
    try {
      const { date } = req.params;
      
      // Get ONLY Daily Capacity Summary data for employee availability
      const latestAnalysis = await storage.getLatestCapacityAnalysis();
      if (!latestAnalysis) {
        return res.status(404).json({ error: 'No analysis data found. Please process Excel files first.' });
      }
      ...
      // Process only available employees from Daily Capacity Summary
      const availableEmployees = [];
      
      for (const empData of employeeData.employees) {
        // Daily Capacity Summary availability rules: Employee + Time Window(s)
        if (empData.status !== 'Available' || !empData.timeWindows || empData.timeWindows.length === 0) {
          continue;
        }
        
        // Get employee postcode from CG Data
        const empLocation = employeeLocations.find(loc => loc.employeeName === empData.employee);
        if (!empLocation || !empLocation.homePostcode || !empLocation.homeLat || !empLocation.homeLng) {
          continue;
        }
        
        // Calculate best client matches within 15-minute travel constraint
        const clientMatches = [];
        ...
        for (const client of clientLocations) {
          ...
          // Estimate travel time (walking: 5km/h = 12min/km, driving: 20km/h = 3min/km)
          const travelTimeMinutes = Math.round(distance * (empLocation.transportMode === 'walking' ? 12 : 3));
        ...
        // Sort by travel time (closest first) and take top 3
        const bestMatches = clientMatches
          .sort((a, b) => a.travelTimeMinutes - b.travelTimeMinutes)
          .slice(0, 3);
      ...
      res.status(500).json({ error: 'Daily scheduling failed', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
