...
import { 
  extractEmployeeAvailabilityWindows,
  extractClientVisitWindows,
  updateClientWindowsWithPostcodes,
  groupEmployeeWindowsByBlockAndDistrict,
  groupClientWindowsByBlockAndDistrict,
  findUnmatchedWindowsForDate,
  generateSchedulingSummary
} from "./time-window-extractor";
import { 
  TimeWindowMatching, 
  VisitBlockType, 
  DEFAULT_VISIT_BLOCKS,
  InsertTimeWindowAssignment
} from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // GET /api/time-windows/:date - Get time window matching data for date
  app.get('/api/time-windows/:date', async (req, res) => {
    try {
      const date = req.params.date;
      console.log(`🕒 Processing time windows for date: ${date}`);

      // Get latest processed data
      const latestAnalysis = await storage.getLatestCapacityAnalysis();
      if (!latestAnalysis) {
        return res.status(404).json({ error: 'No processed data available. Please upload files first.' });
      }

      // Extract employee data for the date
      const employeeData = (latestAnalysis.employeesByDate as Record<string, any[]>)[date] || [];
      console.log(`📊 Found ${employeeData.length} employees for ${date}`);

      // Convert to CleanedEmployeeRecord format (assuming the data structure matches)
      const cleanedEmployeeData = employeeData.map((emp: any) => ({
        employeeName: emp.employeeName,
        contractedWeeklyHours: emp.contractedDailyHours * 7, // Approximate
        ...
        hours: emp.hours,
        netCapacity: emp.netCapacity,
        notes: emp.notes || '',
        postCode: emp.postCode || '' // Will be populated from locations
      }));

      // Extract employee availability windows
      const employeeWindows = extractEmployeeAvailabilityWindows(cleanedEmployeeData, date);
      console.log(`👥 Extracted ${employeeWindows.length} employee availability windows`);

      // Get client locations and visits
      const clientLocations = await storage.getAllClientLocations();
      
      // For now, create default client visit windows since we don't have service delivery data in the analysis
      // TODO: Extract from Hours by Service Type when available
      const clientNames = clientLocations.map(loc => loc.clientName);
      const clientWindows = updateClientWindowsWithPostcodes(
        [], // Empty for now, will be populated when service data is available
        clientLocations.map(loc => ({ clientName: loc.clientName, postcode: loc.postcode }))
      );

      console.log(`🏠 Found ${clientWindows.length} client visit windows`);

      // Group by blocks and districts
      const employeesByBlockAndDistrict = groupEmployeeWindowsByBlockAndDistrict(employeeWindows);
      const clientsByBlockAndDistrict = groupClientWindowsByBlockAndDistrict(clientWindows);

      // Get existing assignments for the date
      const assignments = await storage.getTimeWindowAssignmentsByDate(date);

      // Find unmatched windows
      const unmatched = findUnmatchedWindowsForDate(
        employeeWindows,
        clientWindows,
        assignments
      );

      const result: TimeWindowMatching = {
        date,
        blocks: Object.keys(DEFAULT_VISIT_BLOCKS) as VisitBlockType[],
        employeesByBlockAndDistrict,
        clientsByBlockAndDistrict,
        assignments,
        unmatched
      };

      console.log(`✅ Time window matching complete for ${date}`);
      res.json(result);

    } catch (error) {
      console.error('Time window matching error:', error);
      res.status(500).json({ 
        error: 'Time window matching failed', 
        details: error instanceof Error ? error.message : 'Unknown error' 
      });
    }
  });

  // POST /api/scheduling/assignments - Save employee-client assignments
  app.post('/api/scheduling/assignments', async (req, res) => {
    try {
      const assignmentData = req.body as InsertTimeWindowAssignment;
      console.log(`📝 Saving assignment: ${assignmentData.employeeName} -> ${assignmentData.clientName}`);

      // Validate required fields
      if (!assignmentData.date || !assignmentData.employeeName || !assignmentData.clientName) {
        return res.status(400).json({ 
          error: 'Missing required fields: date, employeeName, clientName' 
        });
      }

      // Save the assignment
      const assignment = await storage.saveTimeWindowAssignment(assignmentData);
  ...
  // GET /api/scheduling/assignments/:date - Get assignments for a date
  app.get('/api/scheduling/assignments/:date', async (req, res) => {
    try {
      const date = req.params.date;
      const assignments = await storage.getTimeWindowAssignmentsByDate(date);
  ...
  // DELETE /api/scheduling/assignments/:id - Delete an assignment
  app.delete('/api/scheduling/assignments/:id', async (req, res) => {
    try {
      const id = req.params.id;
      const deleted = await storage.deleteTimeWindowAssignment(id);
