...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Debug endpoint: Compare employees in map vs picker
  app.get('/api/debug/employee-comparison', async (req, res) => {
    try {
      const branchId = await resolveBranch(req);
      
      // Get all employees from locations (these appear on the map)
      ...
      // Get latest capacity analysis (this has employeesByDate with time windows)
      const analysis = await storage.getLatestCapacityAnalysis(branchId);
      ...
      // Apply picker filter logic (same as weekly-plan-tab.tsx line 160)
      const employeeMap = new Map();
      const employeeWeeklyHoursMap = new Map();
      const adHocEmployees = new Set();
      
      // Calculate guaranteed hours per employee
      Object.values(analysis.employeesByDate || {}).forEach(dayEmployees => {
      ...
      // Filter for picker (only employees with time windows, not ad-hoc, and > 0 GH)
      Object.values(analysis.employeesByDate || {}).flat().forEach(emp => {
        if (emp.status === 'Ad-hoc') {
          adHocEmployees.add(emp.employeeName);
        }
        
        // Line 160 logic: time windows + not ad-hoc + guaranteed hours > 0
  ...
  /**
   * Refine forward-travel times for car-driver matched slots using ORS Directions API.
   *
   * Two improvements over the heuristic computed during matching:
   * 1. If next-visit lat/lng are missing but a postcode is present, geocode it first
   *    (using the same geocodeWithFallback cache — usually already warm).
   * 2. Call ORS Directions API for a real road time (falls back to OSRM if ORS key
   *    absent or rate-limited).
   *
   * Only runs for car/driver transport mode.
   * Runs in parallel for all qualifying slots; failures are non-fatal.
   */
