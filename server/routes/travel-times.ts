...
import { safeErrorMessage, resolveBranch, isUkBst, ukScheduleTimeToUtc } from '../utils/helpers';
import { TravelTimeService, travelTimeService } from '../travel-time-service';

export function registerTravelTimesRoutes(app: Express): void {
  app.post('/api/travel-times/batch', async (req, res) => {
    try {
      ...
      logger.info(
        `[Travel Batch] Mode distribution: ${JSON.stringify(modeSummary)}. Schedule: ${weekStart || 'no date'} @${earliestStartTime || '08:00'}`
      );
      ...
      logger.info(
        `[Travel Batch] Returned ${sessionResults.length} travel times for ${validEmployees.length} employees × ${validClients.length} clients. Sources: ${JSON.stringify(travelSources)}`
      );
  ...
  app.post('/api/travel-times/debug-single', async (req, res) => {
    try {
      ...
      const ttTransportType =
        normalizedMode === 'car' ? 'driving' : distKm <= 1.6 ? 'walking' : 'public_transport';
      ...
      const [result, compare] = await Promise.all([
        ...
        travelTimeService.debugCompareBothEndpoints(
      ...
      res.status(500).json({
        error: safeErrorMessage(error, 'Failed to run debug travel time check'),
      });
    }
  });
}
