...
export function registerGeoRoutes(app: Express): void {
  ...
  app.post('/api/routing/distance-matrix', async (req, res) => {
    try {
      ...
      const originsCoords = origins.map((o: { lat: string; lng: string }) => [parseFloat(o.lng), parseFloat(o.lat)]);
      const destinationsCoords = destinations.map((d: { lat: string; lng: string }) => [parseFloat(d.lng), parseFloat(d.lat)]);
