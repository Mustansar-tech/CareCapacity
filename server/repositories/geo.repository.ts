...
/**
 * Bulk fetch cached travel times for a branch that are still "trustworthy":
 * only real-road sources (ORS single directions or ORS Matrix — never a
 * heuristic/haversine fallback) and no older than `cutoff`. Used to hydrate
 * the in-memory session cache once per branch per run so repeat scheduling
 * runs stop re-hitting the ORS API (and its daily quota) for pairs already
 * known from a recent run.
 */
export async function getFreshTravelTimes(branchId: string, mode: TransportMode | string, cutoff: Date): Promise<TravelTimeCache[]> {
  return db.select().from(travelTimeCache).where(
    and(
      eq(travelTimeCache.branchId, branchId),
      eq(travelTimeCache.transportMode, mode as TransportMode),
      inArray(travelTimeCache.source, ['ors', 'ors-matrix']),
      gte(travelTimeCache.cachedAt, cutoff),
    ),
  );
}

/**
 * Bulk upsert travel time cache rows (e.g. an entire ORS Matrix batch result)
 * in a single statement instead of one round trip per pair.
 */
export async function saveTravelTimesBulk(rows: InsertTravelTimeCache[]): Promise<void> {
  if (rows.length === 0) return;
  await db.insert(travelTimeCache).values(rows)
    .onConflictDoUpdate({
      target: [travelTimeCache.branchId, travelTimeCache.fromLat, travelTimeCache.fromLng, travelTimeCache.toLat, travelTimeCache.toLng, travelTimeCache.transportMode],
      set: {
