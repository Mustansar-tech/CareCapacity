...
async function refineForwardTravelWithORS(
  ...
  await Promise.all(
    pending.map(async (p) => {
      ...
      if (p.nextPostcode) {
        ...
        if (geo?.lat && geo?.lng) p.resolvedCoords = { lat: parseFloat(String(geo.lat)), lng: parseFloat(String(geo.lng)) };
      }
    }),
  );
  ...
  for (const p of pending) {
    ...
    if (p.isCar) {
      if (heuristicMins >= 30) { logger.info(`[FWD-ORS] ${p.slot.day} ${p.slot.availableWindow}: car heuristic=${heuristicMins}min ≥30 — rejected`); rejectedSlots.add(p.slot); }
      else { p.slot.forwardTravelMinutes = heuristicMins; orsQueue.push(p); }
    } else {
      if (heuristicMins > p.gapMins + 20) { logger.info(`[FWD-ORS] ${p.slot.day} ${p.slot.availableWindow}: walk heuristic=${heuristicMins}min >gap+20 — rejected`); rejectedSlots.add(p.slot); }
      else { p.slot.forwardTravelMinutes = heuristicMins; p.slot.forwardTravelWarning = heuristicMins > p.gapMins + 5 ? true : undefined; }
    }
  }
    ...
    try {
      ...
      for (const p of orsQueue) {
        ...
        if (cached) {
          ...
          if (roadMins >= 30) { rejectedSlots.add(p.slot); }
          else { p.slot.forwardTravelMinutes = roadMins; p.slot.forwardTravelWarning = roadMins > p.gapMins + 5; }
        }
      }
    } catch (e) {
  ...
  if (rejectedSlots.size > 0) {
    for (const match of matches) match.matchedSlots = match.matchedSlots.filter(s => !rejectedSlots.has(s));
  }
}

async function buildScheduleMap(branchId: string, analysisDateKeys: string[]) {
  const { storage } = await import('../storage');
...
export async function bdMatch(req: Request, res: Response): Promise<void> {
  ...
  const criteria: ClientEnquiryCriteria = { clientName, postcode, genderPreference: genderPreference || 'any', requiredDays, preferredTimeWindow };
  const analysisDateKeys = Object.keys((selectedAnalysis.employeeSummaryByDate as Record<string, unknown>) || {});
  
  let employeeScheduleMap: Map<string, Map<string, CpVisitEntry[]>> | undefined;
  try {
