...
export async function processCapacity(req: Request, res: Response): Promise<void> {
  ...
  try {
    await scheduleRepo.persistCarerHomeBranchesFromCgData(parsedData.cgData, requestedBranchId);
    logger.info('Persisted carer home-branch mapping from CG Data', { branchId: requestedBranchId, carers: parsedData.cgData.length });
  } catch (mapErr) {
