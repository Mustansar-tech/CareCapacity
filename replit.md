...
- **BD Matrix (Business Development):** A 7-day heatmap visually displaying employee availability for business development opportunities.
  - **Client Enquiry Matcher:** Built-in tool (button in BD Matrix header) that takes client requirements (name, postcode, gender preference, required days, preferred time window, visit duration, weekly hours needed) and algorithmically matches against staff free windows. Returns top 5 ranked employees with exact/adjusted-time/alternative-day match types, remaining weekly capacity, and matched time slots. Backend service in `server/bdMatcher.ts`, API at `POST /api/bd-matcher`.
