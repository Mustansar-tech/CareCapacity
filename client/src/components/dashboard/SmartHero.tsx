...
import { TrendingUp, TrendingDown, Minus, Upload, Bot, Calendar, RefreshCw } from "lucide-react";
...
function buildNarrative(data: ProcessingResult, prevWeekData: CapacityAnalysisSummary | undefined) {
  ...
  const clientRequired = (data.kpis as any).clientRequiredSum ?? 0;
  const clientScheduled = (data.kpis as any).clientScheduledHoursSum ?? 0;
  ...
  const weekendGap = (() => {
    ...
    const weekdays = data.dailySummary.filter(d => {
      const day = new Date(d.date).getUTCDay();
      return day >= 1 && day <= 5;
    });
    const weekend = data.dailySummary.filter(d => {
      const day = new Date(d.date).getUTCDay();
      return day === 0 || day === 6;
    });
  ...
  if (supplyPct < 65) { status = "at risk"; statusColor = "text-red-600 dark:text-red-400"; }
  else if (supplyPct < 82) { status = "under pressure"; statusColor = "text-amber-500 dark:text-amber-400"; }

  let insight = "";
  if (weekendGap !== null && weekendGap > 25) {
    insight = "with a weekend gap worth watching";
  } else if (weekendGap !== null && weekendGap < -10) {
    insight = "with strong weekend coverage";
  } else if (wowPct !== null && wowPct > 3) {
    insight = "with capacity up vs last week";
  } else if (wowPct !== null && wowPct < -3) {
    insight = "with capacity down vs last week";
  } else {
    insight = "holding steady across the week";
  }
...
export function SmartHero({
  ...
  const narrative = useMemo(() => {
    if (!data) return null;
    try { return buildNarrative(data, prevWeekData); } catch (e) {
      clientLogger.error("SmartHero narrative error", e); return null;
    }
  }, [data, prevWeekData]);
                ...
                <h1 className="text-2xl font-bold text-foreground leading-tight mb-1.5 tracking-tight">
                  {(() => {
                    ...
                    return (
                      <>
                        {before}
                        <span className={narrative.statusColor}>{colored}</span>
                        {after}
                      </>
                    );
                  })()}
          ...
          {narrative && (
            <div className="shrink-0 w-56 border border-border rounded-xl bg-card p-4 shadow-sm">
              ...
              <div className="text-xs text-muted-foreground mb-3">
                ...
                {" · "}
                <strong className={narrative.statusColor}>{narrative.supplyPct}%</strong> supply
              </div>
