  ...
  return (
    ...
    >
      {isActive && (
        <span
          className={[
            "absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-r-full bg-amber-400 transition-all",
            collapsed ? "h-5" : "h-5",
          ].join(" ")}
          aria-hidden="true"
        />
      ...
      <Link
        href={href}
        className={[
          "relative flex items-center gap-2.5 rounded-lg mx-2 transition-all duration-150 outline-none select-none",
      ...
      >
        <item.icon className={`w-4 h-4 shrink-0 transition-colors ${isActive ? "text-amber-300" : ""}`} />
        {!collapsed && (
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className={[
            "flex items-center gap-2.5 rounded-lg p-1.5 w-full transition-colors hover:bg-white/10 outline-none",
...
export function Sidebar() {
  ...
  return (
    <>
      <aside
        className={[
          "h-screen flex flex-col shrink-0 transition-all duration-200 overflow-hidden relative z-10",
      ...
      >
        ...
        <div className={[
          ...
          {!collapsed && (
            <button
              ...
              className="p-1.5 rounded-md text-white/35 hover:text-white hover:bg-white/10 transition-colors shrink-0"
        ...
        <div className="px-3 pb-3.5 shrink-0">
          <button
            ...
            className={[
              "w-full flex items-center gap-2 rounded-lg border border-white/10 bg-black/[0.12] text-white/50 hover:text-white hover:bg-black/[0.2] hover:border-white/20 transition-colors",
          ...
          >
            ...
            {!collapsed && (
              <>
                ...
                <kbd className="text-[10px] font-mono bg-white/10 text-white/45 px-1 py-0.5 rounded border border-white/10 leading-none">
                  ⌘K
                </kbd>
              </>
            )}
        ...
        {/* ── Nav items ── */}
        <nav className="flex-1 overflow-y-auto space-y-5 px-0 pt-2 pb-2">
          {/* Pinned section */}
          {pinnedItems.length > 0 && (
            <div>
              {!collapsed && (
                <div className="px-4 pb-1.5 text-[10px] font-semibold uppercase tracking-widest text-amber-200/40">
                  Pinned
                </div>
              )}
          ...
          {visibleGroups.map(group => (
            <div key={group.label}>
              ...
              {collapsed && pinnedItems.length > 0 && (
                <div className="mx-3 mb-1.5 border-t border-white/10" />
              )}
              <div className="space-y-0.5">
        ...
        {/* ── Footer ── */}
        <div className="shrink-0 px-1 pt-2.5 pb-2.5 space-y-1 bg-black/[0.14] border-t border-white/[0.08]">
          {/* Expand button when collapsed */}
          {collapsed && (
            <button
              onClick={toggleCollapsed}
              className="w-full flex justify-center p-2 text-white/35 hover:text-white hover:bg-white/10 rounded-md transition-colors"
          ...
          <button
            ...
            className={[
              "w-full flex items-center gap-2.5 rounded-md px-2 py-1.5 text-white/45 hover:text-white hover:bg-white/10 transition-colors",
              collapsed ? "justify-center" : "",
            ].join(" ")}
